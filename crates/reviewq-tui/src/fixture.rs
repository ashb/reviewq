//! The queue the documentation is drawn from, and the screens it is drawn in.
//!
//! Not test-only, though it began that way: `reviewq help` draws these same
//! screens into the terminal, so a reader of the documentation sees the
//! interface rather than a link to a picture of it. The pictures in `docs/imgs`
//! are the same fixture through [`svg`](crate::svg) instead — see
//! [`docs`](crate::docs), which is where the checking lives.
//!
...
/// Draw one shot at `width`, in `mode`, and hand back the buffer.
///
/// The one drawing path: the SVG in `docs/imgs` and the screen `reviewq help`
/// prints are the same buffer, rendered into different media — so a picture on
/// the project page and a picture in the terminal cannot come to disagree.
pub(crate) fn draw(shot: &Shot, width: u16, mode: Mode) -> (Buffer, Theme) {
    ...
    let buffer = terminal
        ...
        .buffer
        .clone();
    // The app's theme rather than `mode`: a shot may have switched it — the
    // light one exists to do exactly that — and the colours the buffer holds
    // are that theme's.
    (buffer, app.theme)
}

/// The shot of that name, if there is one.
pub(crate) fn shot(name: &str) -> Option<&'static Shot> {
    SHOTS.iter().find(|shot| shot.name == name)
}
...
mod tests {
    ...
    #[test]
    fn every_screen_draws_at_the_width_it_is_asked_for() {
        // The reason this is drawn rather than shipped as a picture: a reader
        // in an 80-column terminal gets an 80-column interface.
        for name in SHOTS.iter().map(|shot| shot.name) {
            for width in [60, 80, 140] {
                let screen = crate::shot(name, width, Mode::Dark, false).expect("a screen");
                let widest = screen.lines().map(|l| l.chars().count()).max().unwrap_or(0);
                assert!(
                    widest <= width as usize,
                    "{name} at {width}: a line came out {widest} wide"
                );
                assert!(screen.contains("reviewq"), "{name} drew its header");
            }
        }
    }

    #[test]
    fn a_name_no_screen_answers_to_is_none() {
        assert!(crate::shot("nonesuch", 100, Mode::Dark, false).is_none());
    }

    #[test]
    fn a_screen_is_plain_text_when_colour_is_not_wanted() {
        let plain = crate::shot("queue", 100, Mode::Dark, false).expect("a screen");
        let coloured = crate::shot("queue", 100, Mode::Dark, true).expect("a screen");
    ...
    #[test]
    fn the_light_screen_is_drawn_in_the_light_palette() {
        // Its whole job: `arrange` switches the theme, and the colours that
        // come back have to be the ones it switched to.
        let light = crate::shot("queue-light", 100, Mode::Dark, true).expect("a screen");
        let dark = crate::shot("queue", 100, Mode::Dark, true).expect("a screen");
